...
class QuantumSineFitter:
    ...
    def get_expectation(self, x):
        # Create the feature map and ansatz circuit
        feature_map = self.create_feature_map(x)
        circuit, ansatz_params = self.create_variational_circuit(feature_map)

        # Assign values from self.params to the circuit parameters
        parameter_values = self.params.detach().numpy().tolist()
        circuit_with_params = circuit.assign_parameters({p: v for p, v in zip(ansatz_params, parameter_values)})

        # Define the observable for expectation
        observable = SparsePauliOp('Z' * self.num_qubits)

        # Run the Estimator to calculate the expectation value
        expectation = self.estimator.run(
            circuits=[circuit_with_params],
...
if __name__ == "__main__":
    ...
    y_train = torch.from_numpy(np.sin(2*np.pi*x_train)).float()
